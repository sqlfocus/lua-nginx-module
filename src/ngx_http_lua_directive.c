...
/* 配置指令"lua_shared_dict <name> <size>;" 
     如"lua_shared_dict dogs 10m;" */
char *
ngx_http_lua_shared_dict(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    /* 注册回调，在ngx_cycle_init()创建共享内存后被调用，用来根据业务初始化 */
...
/* 对应配置指令“rewrite_by_lua_file”解析入口 */
char *
